...
Path: src/etl/extract_transform.py

Parses CME .xlsx files where Contract Month and Option Type
are embedded as sub-headers (e.g., "SEP 26 Calls") above the data tables.
"""
...
def process_cme_directory(raw_folder: Path) -> pd.DataFrame:
    ...
    for file in valid_files:
        ...
        # Read the entire Excel file, preserving empty rows so we can track the structure
        ...
        # Iterate row by row through the Excel file
        for idx, row in df_raw.iterrows():
            ...
            # 1. Detect when we enter the Options section (Skip Futures summary)
            if "OPTION TYPE:" in first_cell.upper():
            ...
            # 2. Detect Sub-Headers (e.g., "SEP 26 Calls" or "DEC 26 Puts")
            # These usually only have one or two populated cells in the row
            if len(row_vals) <= 2 and ('CALL' in first_cell.upper() or 'PUT' in first_cell.upper()):
                parts = first_cell.upper().split()
                # If it looks like "SEP 26 CALLS"
                if len(parts) >= 3:
                    current_contract_month = f"{parts[0]} {parts[1]}"
                    current_option_type = "CALL" if "CALL" in first_cell.upper() else "PUT"
                continue
               
            # 3. Skip the actual table header rows (Strike, Globex, etc.)
            if first_cell.upper() == "STRIKE" or first_cell.upper() == "TOTALS":
                continue
               
            # 4. We found a valid data row (e.g., 3500, 0, 0, 5, 0)
            # Ensure we have our sub-header context and that the first cell is a numeric strike
            if current_contract_month and current_option_type:
                try:
                    strike = float(first_cell.replace(',', ''))
                    # Based on your snippet: Strike is index 0, Volume is index 4, Open Interest is index 8 (At Close)
                    # We will use safer indexing by checking length
                    if len(row_vals) >= 9:
                    ...
                    # First cell wasn't a number, skip row
                    continue
